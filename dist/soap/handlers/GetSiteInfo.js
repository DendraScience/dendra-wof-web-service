...
async function* getSiteInfo(request, {
  ...
  if (!stationId) throw new Error(`Invalid site parameter: ${site}`); // Fetch station and org

  const station = await helpers.findOne('stations', stationId, {
    is_enabled: true,
    is_hidden: false
  });
  const organization = await helpers.findOneCached('organizations', station.organization_id); // Fetch datastreams for station

  const datastreams = (await helpers.findMany('datastreams', {
    is_enabled: true,
    is_hidden: false,
